...
def predict_image(im_name, model1, model2, model3):
    ...
    pred1 = get_prediction(model1, 'vgg16', patches)
    pred2 = get_prediction(model2, 'vgg16', patches)
    pred3 = get_prediction(model3, 'vgg19', patches)
    res = calculate_avg_predictions(pred1, pred2, pred3)
    print()
    print('------------------------------------------------------------')
    return res
...
def calculate_avg_predictions(predictions1, predictions2, predictions3):
    m_score, f_score = 0, 0
    for i in range(len(predictions1)):
        res1 = 0.5 * predictions1[i]
        res2 = 0.4 * predictions2[i]
        res3 = 0.3 * predictions3[i]
        final_res = sum([res1, res2, res3])
